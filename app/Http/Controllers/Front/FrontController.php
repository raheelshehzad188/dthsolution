...
public function view($view, $data = array())
{
    ...
    $assets = rtrim(env('APP_URL'), '/') . '/theme'.$them_num.'/';
    $data['layout'] = $layout;
    ...
    public function categories()
    {
        ...
        {
            $xml .= '<url>
                <loc>'.rtrim(env('APP_URL'), '/').'/'.$post->slug.'</loc> 
                <priority>1.0</priority>
                <changefreq>daily</changefreq>
                </url>';
        }
        ...
        public function product_detail($slug)
{
    ...
    public function loadMoreProducts(Request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            if ($theme === 'theme2') {
                ...
                $template = 'theme4.product_box_new';
            } elseif ($theme === 'front') {
            ...
            $assets_url = rtrim($baseUrl, '/') . '/theme'.$them_num.'/';
    ...
    /**
     * Single-segment URLs: /{slug}
     * Priority: active product → category or subcategory → CMS page → 404
     */
    public function find($slug)
    {
        ...
        if ($product && isset($product->slug) && $product->slug === $slug) {
            return $this->product_detail($slug);
        }

        $hasCategory = Category::where('slug', $slug)->first();
        $hasSubcategory = SubCategory::where('slug', $slug)->first();
    ...
    private function send_grid($to, $subj, $html)
{
    ...
    public function instant_order(Request $request)
    {
        // dd($request);
            ...
            return redirect('/'.$productt->slug)->with([
                'msg'=>'Order submit successfully',
                'msg_type'=>'success',
            ]);
    }
    ...
    public function rating_submit(Request $request)
    {
        // dd($request);
            ...
            return redirect('/'.$product[0]->slug)->with([
    ...
    public function get_selected_shap(Request $request)
    {
            ...
            foreach ($Products as $key => $product) {
                ...
                $html .= '<div class="c-product-grid_item"><div class="c-product-grid_thumb-wrap"><a href="/'.$product->slug.'" class="lrv-loop-product_link"><img data-src="'.$product->image_one.'" src="data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 1 1\'%3E%3C/svg%3E" class="c-product-grid_thumb lazyload lazy-placeholder" loading="lazy"><img data-src="'.$gallary.'" src="data:image/svg+xml,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 1 1\'%3E%3C/svg%3E" class="c-product-grid_thumb product_grid_thumb_hover lazyload lazy-placeholder" loading="lazy"></a><div class="c-product-grid_badges"></div><div class="c-product-grid_thumb-button-list"><button class="h-cb c-product-grid_thumb-button"><i class="ip-eye c-product-grid_icon"></i> <span class="c-product-grid_icon-text">Quick view</span></button> <button data-size="" class="js-wishlist-btn"><i class="ip-heart c-product-grid_icon"></i></button></div></div><a href="/'.$product->slug.'" class="c-product-grid_atc"><span class="c-product-grid_atc-text">View Product</span></a><div class="c-product-grid_details"><div class="c-product-grid_title-wrap"><div class="c-product-grid_category-list"><a class="c-product-grid_category-item" href="/'.$cateslug.'">'.$catename.'</a></div><a href="/'.$product->slug.'" class="product_link_link"><h2 class="lrv-loop-product_title">'.$product->product_name.'</h2></a></div><div class="c-product-grid_price-wrap"><span class="price"><span class="lrv-Price-amount amount"><bdi><span class="lrv-Price-currencySymbol">$</span>'.$product->discount_price.'</bdi></span></span></div></div></div>';
    ...
    public function get_selected_price(Request $request)
    {
        $html = '';
            ...
            foreach ($products as $key => $product) {
                ...
                $html .= '<div class="c-product-grid_item"><div class="c-product-grid_thumb-wrap"><a href="'.url('/'.$product->slug).'" class="lrv-loop-product_link"><img src="/'.$product->image_one.'" class="c-product-grid_thumb"><img src="'.$gallary.'" class="c-product-grid_thumb product_grid_thumb_hover"></a><div class="c-product-grid_badges"></div><div class="c-product-grid_thumb-button-list"><button class="h-cb c-product-grid_thumb-button"><i class="ip-eye c-product-grid_icon"></i> <span class="c-product-grid_icon-text">Quick view</span></button> <button data-size="" class="js-wishlist-btn"><i class="ip-heart c-product-grid_icon"></i></button></div></div><a href="/'.$product->slug.'" class="c-product-grid_atc"><span class="c-product-grid_atc-text">View Product</span></a><div class="c-product-grid_details"><div class="c-product-grid_title-wrap"><div class="c-product-grid_category-list"><a class="c-product-grid_category-item" href="/'.$cateslug.'">'.$catename.'</a></div><a href="/'.$product->slug.'" class="product_link_link"><h2 class="lrv-loop-product_title">'.$product->product_name.'-'.$product->category_id.'</h2></a></div><div class="c-product-grid_price-wrap"><span class="price"><span class="lrv-Price-amount amount"><bdi><span class="lrv-Price-currencySymbol">$</span>'.$product->discount_price.'</bdi></span></span></div></div></div>';
