...
<!-- default -->
    ...
    ->first();?>
                                            <div class="w-100">
                                                <a href="https://api.whatsapp.com/send?phone=<?= $setting->phone?>&amp;text=Hello, I want to purchase:*{{$item->product_name}}* Price:*{{$item->discount_price}} URL:*{{ url('/'); }}/{{$item->slug}}* Thank You !" class="btns-full " style="background-color:green; color: #fff;" id="{{$item->id}}" @if(!$item->status)disabled="true"@endif>Chat On Whatsapp</a>
                                            </div>
                                        </form>
                                    </div>
